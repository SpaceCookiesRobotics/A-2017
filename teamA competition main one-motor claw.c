...
void scoreLeftConeBlue (){
	...
	//drop down to stationary goal
	fallLift(130);//drives lift down for 1.3 seconds
	//release cone
	openGrabber();
...
void scoreRightConeBlue (){//when on blue, turning right (side with loader, less cones)
	fallLift(175); //drives lift down for 2.5
	//turn right
	turnRight(55);//runs for 0.55 seconds
	//move forward
	driveForwDistance(32);
	//grab cone
	closeGrabber(); //grabCone();
	wait10Msec(35);//runs for 10 second for the claw to close
	//rise lift
	riseLift(20, 127); //runs for 0.2 seconds to ensure that the cone if recieved
	//turn left
	turnLeft(84); //runs for 0.85 seconds
	wait10Msec(1);//pauses inbetween the two actions
	//lift up all the way
	riseLift(250, 127);//dirves lift up for 2.5 seconds at full motor power
	//wait10Msec(1);//pauses for 10 milliseconds so they don't run together
	driveForwDistance(22);//drives to stationary goal - runs for longer from where the cone was
	//drop down to stationary goal
	fallLift(130);//drives lift down for 1.3 seconds
	//release cone
	openGrabber();
...
void scoreLeftConeRed (){//when on red, turning left (side with loader, less cones)
	fallLift(175); //drives lift down for 2.5
	//turn left
	...
	riseLift(20, 127); //runs for 0.2 seconds to ensure that the cone if recieved
	//turn right
	turnRight(84);//runs for 0.7 seconds
	wait10Msec(1);//pauses inbetween the two actions
	//lift up all the way
	riseLift(250, 127);//dirves lift up for 2.5 seconds at full motor power
	//wait10Msec(1);//pauses for 10 milliseconds so they don't run together
...
void autonomous() {
	...
	*/
	...
	else if (direction == 0 || direction == 1 && distance == 0 || distance == 1 && cones == 1){//to just score the preload cone
		scorePresetCone();
	}
	else if (direction == 0 && distance == 0 && cones == 2) {//when turning left on the blue alliance, without scoring the preload con//	scoreLeftConeBlue();
	}
	else if (direction == 0 && distance == 0 && cones == 0) {//when turning left on the blue alliance
		scorePresetCone();
		wait10Msec(1);
		scoreLeftConeBlue();
	}
	else if (direction == 1 && distance == 1 && cones == 2) {//when turning right on the blue alliance, without scoring the preload cone
		scoreRightConeBlue();
	}
	 else if (direction == 1 && distance == 1 && cones == 0) {//when turning right on the blue alliance
		scorePresetCone();
		wait10Msec(1);
		scoreRightConeBlue();
	}
	else if (direction == 1 && distance == 1 && cones == 2) {//when turning right on the blue alliance, without scoring te preload cone
		scoreRightConeBlue();
	}
	else if (direction == 1 && distance == 0 && cones == 0) {//when turning right on the red alliance
		scorePresetCone();
		wait10Msec(1);
		scoreRightConeRed();
	}
	else if (direction == 0 && distance == 1 && cones == 2) {//when turning left on the red alliance, without scoring the preload cone
		scoreLeftConeRed();
	}
	else if (direction == 0 && distance == 1 && cones == 0) {//when turning left on the red alliance
		scorePresetCone();
		wait10Msec(1);
		scoreLeftConeRed();
	}
